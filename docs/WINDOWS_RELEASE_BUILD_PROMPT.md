...
> Skill v0.6.2** but cannot build or natively verify the **Windows** offline runtime — that requires
...
- **Current state:** main is at v0.6.2. A GitHub **prerelease `v0.6.2`** already exists carrying the
  **macOS arm64** archive (`horosa-runtime-darwin-arm64-v0.6.2.tar.gz`) + `SHA256SUMS.txt`. The
  **Windows x64** archive is **missing** — that is what you are producing. `releases/latest` is still
  `v0.5.13`, so nothing public is broken while you work.
...
1. Build `horosa-skill/dist/runtime/horosa-runtime-win32-x64-v0.6.2.zip`.
...
4. Upload the Windows zip (+ refreshed manifest/checksums) to the `v0.6.2` GitHub release, then — only
...
- **win32 wheels** in that `wheels/` dir. They MUST include the ken deps **`bidict`, `numpy`,
  ...
  start. If the user keeps a Windows source tree, `HOROSA_WINDOWS_SOURCE_ROOT=<that-tree> bash
  horosa-skill/scripts/sync_vendored_runtime_sources.sh` populates `vendor/runtime-source/runtime/windows`.
  If they don't, ask them how their win32 wheels are produced (typically `pip download --only-binary=:all:
  --platform win_amd64 --python-version 311` of the dep set, or building on this machine).

## 3. Build the Windows runtime

```powershell
# from the repo root
git fetch origin; git checkout main; git pull        # must include v0.6.2 (pyproject version == 0.6.2)
cd horosa-skill
uv sync
uv run python -c "from horosa_skill import __version__; print(__version__)"   # expect 0.6.2
...
dir dist\runtime\horosa-runtime-win32-x64-v0.6.2.zip
```
...
Expand-Archive dist\runtime\horosa-runtime-win32-x64-v0.6.2.zip -DestinationPath $dst
$payload = Join-Path $dst "runtime-payload"

# (a) embedded manifest must read 0.6.2
...
tongshefa returns `right_elem=金 / main_relation=实克思`; the embedded manifest says `0.6.2`.
...
The macOS archive already exists on the `v0.6.2` release — download it next to the Windows zip so the
manifest and `SHA256SUMS.txt` cover both platforms.

```powershell
cd horosa-skill
gh release download v0.6.2 --repo Horace-Maxwell/horosa-skill `
  --pattern "horosa-runtime-darwin-arm64-v0.6.2.tar.gz" --dir dist\runtime

uv run python scripts/generate_release_manifest.py `
  --version 0.6.2 `
  --darwin-archive dist\runtime\horosa-runtime-darwin-arm64-v0.6.2.tar.gz `
  --darwin-url https://github.com/Horace-Maxwell/horosa-skill/releases/latest/download/horosa-runtime-darwin-arm64-v0.6.2.tar.gz `
  --windows-archive dist\runtime\horosa-runtime-win32-x64-v0.6.2.zip `
  --windows-url https://github.com/Horace-Maxwell/horosa-skill/releases/latest/download/horosa-runtime-win32-x64-v0.6.2.zip `
  --output dist\runtime\runtime-manifest.json

# checksums over both archives (regenerate SHA256SUMS.txt for both)
cd dist\runtime
(Get-FileHash horosa-runtime-darwin-arm64-v0.6.2.tar.gz -Algorithm SHA256).Hash.ToLower() + "  horosa-runtime-darwin-arm64-v0.6.2.tar.gz" | Out-File SHA256SUMS.txt -Encoding ascii
(Get-FileHash horosa-runtime-win32-x64-v0.6.2.zip -Algorithm SHA256).Hash.ToLower() + "  horosa-runtime-win32-x64-v0.6.2.zip" | Out-File SHA256SUMS.txt -Append -Encoding ascii
cd ..\..
...
## 6. Finalize the v0.6.2 release (confirm with the user first)

```powershell
gh release upload v0.6.2 --repo Horace-Maxwell/horosa-skill `
  horosa-skill\dist\runtime\horosa-runtime-win32-x64-v0.6.2.zip `
  horosa-skill\dist\runtime\runtime-manifest.json `
  horosa-skill\dist\runtime\SHA256SUMS.txt --clobber

# ONLY after the user confirms they want v0.6.2 to become the public latest:
gh release edit v0.6.2 --repo Horace-Maxwell/horosa-skill --draft=false --prerelease=false --latest
```
...
- **Report back to the user** with: the Windows zip SHA256, the three ken endpoint results, the tongshefa
  result, `verify_runtime_release.py` output, and whether you flipped v0.6.2 to latest. If you changed
